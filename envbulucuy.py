...
class EnhancedEnvironmentScanner:
    ...
    def setup_patterns(self):
        ...
        self.env_patterns = [
            (r'\bos\.getenv\(["\']([^"\']+)["\']\s*,\s*["\']([^"\']*)["\']\)', 'os.getenv_default'),
    ...
    def _ast_analysis(self, tree: ast.AST, results: Dict, file_path: str):
        ...
        class EnvVisitor(ast.NodeVisitor):
            ...
            def visit_Assign(self, node):
                ...
                if isinstance(node.value, ast.Call) and isinstance(node.value.func, ast.Attribute):
                    if node.value.func.attr == 'getenv' and isinstance(node.value.func.value, ast.Name) and node.value.func.value.id == 'os':
                        try:
                            var_name = node.value.args[0].value  # Constant node için .value
                            default_value = node.value.args[1].value if len(node.value.args) > 1 else ""
                ...
                if isinstance(node.value, ast.Call) and (isinstance(node.value.func, ast.Attribute) or isinstance(node.value.func, ast.Name)):
                    func_name = ""
                    ...
                    if func_name in ['join', 'Path']:
                        try:
                            ...
                            full_path = os.path.join(*path_parts).replace('\\', '/')
                            var_name = node.targets[0].attr if isinstance(node.targets[0], ast.Attribute) else node.targets[0].id
                            self.results['env_vars'][var_name] = {
                                'value': full_path,
                                'sources': [f"{self.file_path}:{node.lineno}"]
                            }
        ...
        # Ziyaretçiyi çağır
        EnvVisitor(results, file_path).visit(tree)  # Bu satır metodun içinde olmalı
